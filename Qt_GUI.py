...
from src.GCodeSender import GCodeSenter, DEVICES, SomeGCodes  # Assuming gcode code is in gcode_module.py
...
class AxisControlWidget(QWidget):
    ...
    def move_minus(self):
        try:
            current_pos = self.gcode_sender.get_pos()[{"X": 0, "Y": 1, "Z": 2}[self.axis_name]]
    ...
    #     current_pos = positions[self.axis_name]
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Two camera feeds (adjust camera IDs as needed)
        # Controls section
        ...
        header_layout.addWidget(QLabel("Axis"))
        header_layout.addWidget(QLabel("Target"))
        header_layout.addWidget(QLabel(""))  # spacer for buttons
        header_layout.addWidget(QLabel(""))  # spacer for buttons
        ...
        main_layout.addWidget(controls_group)  # 1/3 of space
        
    def connect_device(self):
        try:
            self.gcode_sender.connect()
            self.gcode_sender.send_command(SomeGCodes.SET_STEPS_PER_UNIT_GCODE, need_to_await=False)
            self.gcode_sender.send_command(SomeGCodes.SET_CURRENT_LIMIT_GCODE, need_to_await=False)
            self.gcode_sender.send_command(SomeGCodes.SET_MAX_FEEDRATE_GCODE, need_to_await=False)
            self.gcode_sender.send_command(SomeGCodes.SET_ACCELERATION_GCODE, need_to_await=False)
            self.gcode_sender.send_command(SomeGCodes.SET_MICROSTEPPING_GCODE, need_to_await=False)
    ...
    def move_to_pos_3(self):
        # x = float(self.position_3_x.text())
        # y = float(self.position_3_y.text())
        z = float(self.position_3_z.text())

        # self.widget_x.target_edit.setText(x)
        # self.widget_y.target_edit.setText(y)
        self.widget_z.target_edit.setText(f"{z:.2f}")
